...
##CREATE 10,000 matrix # I just recicled the matrix because are random values 

test_4 <- matrix(
  rtruncnorm(10000 * 1000, a = -2.5, b = 2.5, mean = 0, sd = 1), 
  nrow = 10000, 
  ncol = 1000
)
...
#psuedys9_4 <- ys9_4 + stdevs2_9_4 * test_4  # this makes the new biomases if no heteroscedasticity #

psuedys4 <- ys4 + stdevs2_4 * test_4 * dbh2_4 # This makes the new biomasses if heteroscedasticity
...
write.csv(coefficients4, 
          file = "1_Coefficients_Species_Uncertainty/coefficients_4.csv", 
          row.names = FALSE)

write.csv(PseudoDataTsugaCa1, 
          file = "1_Coefficients_Species_Uncertainty/TsugaCa.csv", 
          row.names = FALSE)
